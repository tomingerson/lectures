...
public class DatabaseConnector implements CommandLineRunner {
    ...
    public void run(String... args) throws ClassNotFoundException {
        ...
        try {
            ...
            stm.execute("CREATE TABLE customers(id SERIAL, first_name VARCHAR(255)," +
                    " last_name VARCHAR(255))");

            final PreparedStatement pstm = con.prepareStatement("INSERT INTO customers(first_name, " +
                    "last_name) " +
                    "VALUES (?,?)");

            // some database guys
            Stream.of("Arun Gupta","Lukas Eder", "Josh Juneau", "Josh Long", "Vlad Mihalcea")
                    .map(name -> name.split(" "))
                    .forEach(sa -> {
                        ...
                        try {
                            ...
                            logger.error("error creating a prepared statement with arguments '" + fn +
                                    "' and" + " '" + ln + "'", e);
                        }
                    });
            pstm.executeBatch();
            ...
            final PreparedStatement pstmRead = con.prepareStatement("SELECT id, first_name, last_name" +
                    " FROM customers WHERE first_name = ?");
            pstmRead.setString(1, "Josh");
            final ResultSet rs = pstmRead.executeQuery();
            while (rs.next()) {
                final Person customer = new Person(rs.getLong("id"), rs.getString("first_name"),
